...
/**
 * Base Sepolia Testnet Deployment Module
 *
 * Key differences from local (FullSystem.ts):
 * - Deploys mock BTC/USD and cbBTC/BTC feeds for repeatable Base Sepolia tests
 * - Uses our own UniswapV2Pair contracts for LP pools (simpler than using official Factory)
 * - Token strategy:
 *   - WETH: Uses the OP Stack WETH predeploy
 *   - cbBTC/USDC/USDT: Deploys our own mock tokens for repeatable testing
 *
 ...
 *
 * Deployment order:
 * 1. Deploy tokens (cbBTC, BTD, BTB, BRS, USDC, USDT) + use WETH predeploy
...
// Base Sepolia uses internal LP pairs for protocol bootstrap.
export const BASE_SEPOLIA_UNISWAP_V2 = {
  FACTORY: "0x0000000000000000000000000000000000000000",
  ROUTER: "0x0000000000000000000000000000000000000000",
  WETH9: BASE_SEPOLIA.WETH,
};
...
export default buildModule("FullSystemBaseSepolia", (m) => {
  const deployer = m.getAccount(0);

  // ===== Phase 1: Tokens =====

  // 1.1 Deploy mock cbBTC/USDC/USDT. The WBTC id is retained for address-map compatibility.
  const wbtc = m.contract("contracts/local/MockCbBTC.sol:MockCbBTC", [deployer], { id: "WBTC" });
  ...
  // Use the OP Stack WETH predeploy on Base Sepolia.
  const weth = m.contractAt("contracts/interfaces/IWETH9.sol:IWETH9", BASE_SEPOLIA.WETH, { id: "WETH" });
  ...
  // ===== Phase 6: Oracles =====
  // Mock BTC/USD oracle for deterministic Base Sepolia validation.
  ...
  // Mock cbBTC/BTC oracle, assume 1:1.
  ...
  // Transfer tokens to Faucet: 10M cbBTC, 500M USDC, 500M USDT
  ...
  return {
    ...
    uniswapV2: BASE_SEPOLIA_UNISWAP_V2,
  };
});
