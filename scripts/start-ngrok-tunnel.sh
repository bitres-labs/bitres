...
if [ ! -f ~/.config/ngrok/ngrok.yml ]; then
    print_info "First run requires ngrok authtoken"
    echo ""
    echo -e "${YELLOW}Visit https://dashboard.ngrok.com/get-started/your-authtoken${NC}"
    echo -e "${YELLOW}Sign up for a free account and get your authtoken${NC}"
    echo ""
...
echo -e "  ${GREEN}2. Copy that HTTPS address${NC}"
echo -e "  ${GREEN}3. Add a network in MetaMask:${NC}"
echo ""
echo -e "     ${BLUE}Network name:${NC} BRS ngrok"
echo -e "     ${BLUE}RPC URL:${NC} https://xxxx.ngrok.io (use the forwarding URL)"
