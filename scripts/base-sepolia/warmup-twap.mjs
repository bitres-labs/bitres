/**
 * TWAP Warmup Script for Base Sepolia
 *
 ...
 *
 * How TWAP works:
 * - First observation is recorded during init-base-sepolia.mjs
 * - This script records the second observation after 30 min
 * - With 2 observations >= 30 min apart, TWAP prices become available
 *
 * Usage:
 *   npx hardhat run scripts/base-sepolia/warmup-twap.mjs --network baseSepolia
 *   npm run base-sepolia:warmup-twap
 */
...
const ADDR_FILE = path.join(process.cwd(), "ignition/deployments/chain-84532/deployed_addresses.json");
...
const PRICE_ORACLE_ABI = [
  ...
  { inputs: [{ name: "_useTWAP", type: "bool" }], name: "setUseTWAP", outputs: [], stateMutability: "nonpayable", type: "function" },
  { inputs: [], name: "getWBTCPrice", outputs: [{ type: "uint256" }], stateMutability: "view", type: "function" },
  { inputs: [], name: "getBTCCollateralPrice", outputs: [{ type: "uint256" }], stateMutability: "view", type: "function" },
...
async function main() {
  ...
  for (const [key, value] of Object.entries(raw)) {
    const name = key.replace("FullSystemBaseSepolia#", "");
    addr[name] = value;
  }
  ...
  if (allReady) {
    ...
    if (useTWAP) {
      const hash = await walletClient.writeContract({
        address: addr.PriceOracle,
        abi: PRICE_ORACLE_ABI,
        functionName: "setUseTWAP",
        args: [false],
        gas: 100000n,
      });
      const receipt = await publicClient.waitForTransactionReceipt({ hash });
      if (receipt.status !== "success") throw new Error(`setUseTWAP(false) failed: ${hash}`);
      console.log("   TWAP mode was enabled too early; disabled until warmup is ready.\n");
    }
    return;
  } else {
    ...
    for (const pair of pairs) {
      ...
      if (needsUpdate) {
        ...
        const receipt = await publicClient.waitForTransactionReceipt({ hash });
        if (receipt.status !== "success") throw new Error(`TWAP update failed: ${hash}`);
  ...
  if (!useTWAP) {
    console.log("=> Enabling TWAP mode...\n");
    const hash = await walletClient.writeContract({
      address: addr.PriceOracle,
      abi: PRICE_ORACLE_ABI,
      functionName: "setUseTWAP",
    ...
    const receipt = await publicClient.waitForTransactionReceipt({ hash });
    if (receipt.status !== "success") throw new Error(`setUseTWAP(true) failed: ${hash}`);
    console.log(`   ✓ TWAP enabled (tx: ${hash.slice(0, 10)}...)\n`);
  }
  ...
  try {
    const wbtcPrice = await publicClient.readContract({
      address: addr.PriceOracle,
      abi: PRICE_ORACLE_ABI,
      functionName: "getBTCCollateralPrice",
    });
    console.log(`   cbBTC: $${(Number(wbtcPrice) / 1e18).toLocaleString()}`);
