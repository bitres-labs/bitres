 ...
 *
 * Distribute test tokens to specified addresses on Base Sepolia testnet.
 *
 * Usage:
...
async function main() {
  ...
  console.log("  Bitres Base Sepolia Token Faucet");
  console.log("=".repeat(60));
  ...
  if (recipients.length === 0) {
    ...
    console.log("  FAUCET_RECIPIENTS=0x123...,0x456... npx hardhat run scripts/base-sepolia/faucet.mjs --network baseSepolia");
  ...
  const tokens = {
    WBTC: {
      ...
      abi: loadAbi("contracts/local/MockCbBTC.sol/MockCbBTC.json"),
      amount: AMOUNTS.WBTC,
      decimals: 8,
    },
    USDC: {
