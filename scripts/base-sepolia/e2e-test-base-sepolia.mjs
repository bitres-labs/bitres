/**
 * Base Sepolia E2E Test Script
 *
 * Verifies core functionality on Base Sepolia testnet after deployment.
 * Uses deployer wallet for write operations.
 *
 * Run:
 *   npx hardhat run scripts/base-sepolia/e2e-test-base-sepolia.mjs --network baseSepolia
 */
...
function loadAddresses() {
  ...
  for (const [k, v] of Object.entries(raw)) {
    map[k.replace("FullSystemBaseSepolia#", "")] = v;
  }
  return map;
}
...
async function main() {
  ...
  console.log("  Bitres Base Sepolia E2E Test");
  console.log("=".repeat(70));
  ...
  const rpcUrl = hre.network.config?.url || process.env.BASE_SEPOLIA_RPC_URL;
  ...
  const erc20Abi = loadAbi("contracts/local/MockCbBTC.sol/MockCbBTC.json");
  const stBTDAbi = loadAbi("contracts/stBTD.sol/stBTD.json");

  const waitFor = async (label, fn, predicate, attempts = 12) => {
    let value;
  ...
  const readTokenBalance = (token, account) =>
  ...
  const waitForAllowance = (token, spender, amount) =>
    waitFor(
  ...
  const btcPrice = await priceOracle.read.getBTCCollateralPrice();
  assert(btcPrice > 0n, `cbBTC price = ${formatEther(btcPrice)} (> 0)`);
  ...
  const mintWbtcAmt = parseUnits("0.001", 8); // 0.001 cbBTC (conserve test tokens)
  ...
  console.log(
    `   info: Deployer cbBTC balance = ${formatUnits(deployerWbtcBal, 8)}`
  );

  if (deployerWbtcBal >= mintWbtcAmt) {
    try {
    // Approve Minter to spend cbBTC
    ...
    await waitForAllowance(addresses.WBTC, addresses.Minter, mintWbtcAmt);

    // Calculate expected mint amount
    const [expectedBtd, expectedFee] = await minter.read.calculateMintCollateralAmount([
      mintWbtcAmt,
    ]);
    assert(
      expectedBtd > 0n,
      `0.001 cbBTC mints ${formatEther(expectedBtd)} BTD (fee: ${formatEther(expectedFee)})`
    );

    // Mint BTD
    const btdBalBefore = await readTokenBalance(addresses.BTD, deployer.account.address);
    await writeAndWait(deployer, {
      address: addresses.Minter,
      abi: minterAbi,
      functionName: "mintBTDWithCollateral",
      args: [mintWbtcAmt],
    });
    const btdBalAfter = await waitFor(
      "BTD balance after mint",
      () => readTokenBalance(addresses.BTD, deployer.account.address),
      (balance) => balance >= btdBalBefore + expectedBtd
    );
    ...
    console.log(
      `   SKIP: Deployer cbBTC balance too low (${formatUnits(deployerWbtcBal, 8)}), skipping mint test`
    );
  }
  ...
  if (deployerBtdBal >= redeemAmount) {
    ...
    await waitForAllowance(addresses.BTD, addresses.Minter, redeemAmount);

    const [expectedWbtc, redeemFeeAmt] =
      await minter.read.calculateBurnAmount([redeemAmount]);
    assert(
      expectedWbtc > 0n,
      `Redeeming ${formatEther(redeemAmount)} BTD returns ${formatUnits(expectedWbtc, 8)} cbBTC (fee: ${formatUnits(redeemFeeAmt, 8)})`
    );

    const wbtcBefore = await readTokenBalance(addresses.WBTC, deployer.account.address);
    await writeAndWait(deployer, {
      address: addresses.Minter,
      abi: minterAbi,
    ...
    const wbtcAfter = await waitFor(
      "cbBTC balance after redeem",
      () => readTokenBalance(addresses.WBTC, deployer.account.address),
      (balance) => balance > wbtcBefore
    );
    const wbtcReceived = wbtcAfter - wbtcBefore;
    assert(
      wbtcReceived > 0n,
      `Deployer received ${formatUnits(wbtcReceived, 8)} cbBTC from redemption`
    );
    } catch (e) {
  ...
  if (deployerUsdcBal >= usdcAmount) {
    ...
    await waitForAllowance(addresses.USDC, addresses.FarmingPool, usdcAmount);
    ...
    const [stakedAmount] = await waitFor(
      "USDC farming stake",
      () => farming.read.userInfo([3n, deployer.account.address]),
      ([amount]) => amount >= stakedBefore + usdcAmount
    );
    assert(
      stakedAmount >= stakedBefore + usdcAmount,
      `Deployer staked ${formatUnits(stakedAmount - stakedBefore, 6)} USDC in pool 3`
    );
    ...
    // Note: On Base Sepolia, withdraw may fail due to reward claim gas estimation issues
    ...
    try {
      ...
      const [stakedAfterWithdraw] = await waitFor(
        "USDC farming withdraw",
        () => farming.read.userInfo([3n, deployer.account.address]),
        ([amount]) => amount <= stakedAmount - actualDeposited
      );
      assert(
      ...
      console.log(`   info: Withdraw reverted (known Base Sepolia gas issue), skipping`);
  ...
  if (deployerBtdForVault >= vaultDeposit) {
    ...
    await waitForAllowance(addresses.BTD, addresses.stBTD, vaultDeposit);

    const sharesBefore = await readTokenBalance(addresses.stBTD, deployer.account.address);
    await writeAndWait(deployer, {
    ...
    const sharesAfter = await waitFor(
      "stBTD shares after deposit",
      () => readTokenBalance(addresses.stBTD, deployer.account.address),
      (balance) => balance > sharesBefore
    );
    ...
    const btdBeforeRedeem = await readTokenBalance(addresses.BTD, deployer.account.address);
    await writeAndWait(deployer, {
      address: addresses.stBTD,
      abi: stBTDAbi,
      functionName: "redeem",
    ...
    const btdAfterRedeem = await waitFor(
      "BTD balance after stBTD redeem",
      () => readTokenBalance(addresses.BTD, deployer.account.address),
      (balance) => balance > btdBeforeRedeem
    );
    assert(
  ...
  assert(
    ...
    `ConfigCore.WBTC legacy alias matches deployed cbBTC address`
  );

  const coreBtcCollateral = await configCore.read.BTC_COLLATERAL();
  assert(
    coreBtcCollateral.toLowerCase() === addresses.WBTC.toLowerCase(),
    `ConfigCore.BTC_COLLATERAL matches deployed cbBTC address`
  );
  ...
  assert(
    treasuryWbtcBal > 0n,
    `Treasury cbBTC balance = ${formatUnits(treasuryWbtcBal, 8)} (> 0)`
  );

  const [tWbtc, tBrs, tBtd] = await treasury.read.getBalances();
  assert(tWbtc >= 0n, `Treasury getBalances returned successfully`);
  console.log(
    `   info: Treasury cbBTC = ${formatUnits(tWbtc, 8)}, BRS = ${formatEther(tBrs)}, BTD = ${formatEther(tBtd)}`
  );
  ...
  if (failed > 0) {
    ...
    console.log("\n  All tests passed! Base Sepolia deployment is functional.");
  }
}
